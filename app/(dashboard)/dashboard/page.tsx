...
The briefing should be persona-based (Strux Assistant), encouraging, and focus on 1-2 key things for today.
  ...
  const TrendIndicator = ({ trend, label }: { trend: string, label: string }) => {
    ...
    return <span className="text-[8px] font-black text-slate-500 uppercase flex items-center gap-0.5"><Minus size={10} /> Stable</span>;
  };

  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-10 pb-32 bg-slate-950 text-slate-100 min-h-screen">
      {/* Welcome Bar */}
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <h1 className="text-3xl font-black tracking-tight bg-gradient-to-br from-white to-slate-500 bg-clip-text text-transparent">
        ...
        {memory?.goals && (
          <div className="px-5 py-3 bg-primary/5 border border-primary/20 rounded-2xl shadow-xl shadow-primary/5">
            ...
            <p className="text-xs font-bold text-slate-300">"{memory.goals}"</p>
          </div>
        )}
      </header>

      {/* AI Daily Briefing */}
      <div className="bg-slate-900 border border-slate-800 rounded-[2.5rem] p-8 shadow-2xl flex flex-col md:flex-row gap-8 items-center group hover:border-primary/30 transition-all">
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between px-2">
          <h2 className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-600">Performance Trends</h2>
          <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">vs Last Week</span>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {/* Card 1: Total Tasks */}
          <div className="p-6 bg-slate-900 border border-slate-800 rounded-[2rem] space-y-4 hover:bg-slate-800/50 transition-all border-b-4 border-b-blue-500/50">
            ...
            <div>
              ...
              <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Velocity Items</p>
            </div>
          </div>

          {/* Card 2: Completion Rate */}
          <div className="p-6 bg-slate-900 border border-slate-800 rounded-[2rem] space-y-4 hover:bg-slate-800/50 transition-all border-b-4 border-b-emerald-500/50">
            <div className="flex justify-between items-start">
            ...
            <div>
              ...
              <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Efficiency Mark</p>
            </div>
          </div>

          {/* Card 3: Notes (Knowledge) */}
          <div className="p-6 bg-slate-900 border border-slate-800 rounded-[2rem] space-y-4 hover:bg-slate-800/50 transition-all border-b-4 border-b-purple-500/50">
            <div className="flex justify-between items-start">
            ...
            <div>
              ...
              <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Knowledge Base</p>
            </div>
          </div>

          {/* Card 4: Focus Time */}
          <div className="p-6 bg-slate-900 border border-slate-800 rounded-[2rem] space-y-4 hover:bg-slate-800/50 transition-all border-b-4 border-b-orange-500/50">
            <div className="flex justify-between items-start">
            ...
            <div>
              ...
              <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Deep Work Minutes</p>
            </div>
          </div>
        </div>

        {/* Summary Line */}
        <div className="px-6 py-4 bg-slate-900/30 rounded-2xl border border-slate-800/50 flex items-center gap-3">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        <div className="lg:col-span-2 space-y-6">
          <div className="flex items-center justify-between px-2">
            ...
            <Link href="/tasks" className="text-[10px] font-black text-primary hover:text-white uppercase tracking-widest transition-colors">View Deep Stack</Link>
          </div>
          
          <div className="bg-slate-900 border border-slate-800 rounded-[2.5rem] overflow-hidden shadow-2xl">
            {thisWeekTasks.filter(t => t.status !== 'done').length > 0 ? (
              thisWeekTasks.filter(t => t.status !== 'done').slice(0, 5).map(task => (
                <div key={task.id} className="p-6 flex items-center justify-between group hover:bg-slate-800/50 transition-all border-b border-slate-800/50 last:border-0 cursor-pointer">
                  <div className="flex items-center gap-4">
                    <div className="h-6 w-6 rounded-lg border-2 border-slate-800 group-hover:border-primary/50 transition-all flex items-center justify-center">
                        <div className="h-2 w-2 rounded-sm bg-primary opacity-0 group-hover:opacity-100 transition-all" />
                    </div>
                    <span className="text-sm font-bold text-slate-100">{task.title}</span>
                  </div>
                  {task.due_date && (
                    <span className="text-[8px] font-black px-3 py-1 rounded-full bg-slate-800 text-slate-400 uppercase tracking-widest border border-slate-700">
                      {task.due_date}
                    </span>
                  )}
                </div>
              ...
              <div className="p-20 text-center space-y-4">
                ...
                <p className="text-xs font-black uppercase tracking-widest text-slate-600">Roadmap clear. Excellent execution.</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="space-y-6">
          <h2 className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-600 px-2">Command Center</h2>
          <div className="grid gap-4">
            {[
              ...
              <Link 
                key={i} 
                href={action.href} 
                className="flex items-center gap-4 p-5 bg-slate-900 border border-slate-800 rounded-2xl hover:bg-slate-800 hover:border-slate-700 transition-all group relative overflow-hidden"
              >
                <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/5 to-transparent translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-700" />
                <div className="h-10 w-10 rounded-xl bg-slate-950 flex items-center justify-center text-slate-500 group-hover:text-primary transition-colors shadow-inner">
                  <action.icon size={20} />
                </div>
                <span className="text-[10px] font-black text-slate-200 uppercase tracking-[0.2em]">{action.label}</span>
              </Link>
            ))}
          </div>
        </div>
